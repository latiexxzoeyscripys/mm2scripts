...
local function base32_encode(data)
    local bits = ""
    for i = 1, #data do
        bits = bits .. string.format("%08b", data:byte(i))
    end
    local pad = (5 - (#bits % 5)) % 5
    bits = bits .. string.rep("0", pad)
    local out = {}
    for i = 1, #bits, 5 do
        local chunk = bits:sub(i, i + 4)
        local index = tonumber(chunk, 2) + 1
        table.insert(out, b32chars:sub(index, index))
    end
    while (#out % 8) ~= 0 do
        table.insert(out, "=")
    end
...
local function base32_decode(b32)
    ...
    for i = 1, #b32 do
        local c = b32:sub(i,i)
        local index = (b32chars:find(c) or 1) - 1
        bits = bits .. string.format("%05b", index)
    end
    local out = {}
    for i = 1, #bits, 8 do
